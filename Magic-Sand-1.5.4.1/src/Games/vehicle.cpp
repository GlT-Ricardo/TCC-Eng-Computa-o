...
void Fish::draw()
{
    ...
    if (mother)
    {
        c.setHsb(0-(int)hsb, 0, 0); // rainbow
        ofSetColor(c);
    }
...
void Rabbit::draw()//, std::vector<ofVec2f> forces)
{
	...
	/* Arbusto
    ofSetColor(c2);
    ofDrawCircle(0*sc, 20, 3.5*sc);
	ofDrawCircle(0 * sc, -20, 3.5 * sc);
	ofDrawCircle(-5 * sc, 0, 3.5 * sc);
	ofDrawCircle(5 * sc, 0, 3.5 * sc);
	ofSetColor(c3);
	ofDrawCircle(3.5 * sc, 15, 3.5 * sc);
	ofDrawCircle(-3.5 * sc, 15, 3.5 * sc);
	ofDrawCircle(-3.5 * sc, -15, 3.5 * sc);
	ofDrawCircle(3.5 * sc, -15, 3.5 * sc);*/
	...
	body.curveTo(ofPoint(-2 * sc, 5.5*sc));
	body.curveTo(ofPoint(-9 * sc, 0.5*sc));
	body.curveTo(ofPoint(-8 * sc, 0 * sc));
	body.curveTo(ofPoint(-9 * sc, -0.5*sc));
	body.curveTo(ofPoint(-2 * sc, -5.5*sc));
	body.curveTo(ofPoint(-2 * sc, -5.5 * sc));
	//body.curveTo(ofPoint(2 * sc, 0 * sc));
	body.close();
	ofSetColor(c1);
	...
	ofPath head;
	head.curveTo(ofPoint(-10, 0*sc));
	head.curveTo(ofPoint(-1, 1.5*sc));
	head.curveTo(ofPoint(-1, 1.5*sc));
	head.curveTo(ofPoint(-0.5 * sc, 1.5*sc));
	head.curveTo(ofPoint(-5 * sc, 5.5*sc));
	head.curveTo(ofPoint(0, 5.5*sc));
	head.curveTo(ofPoint(5 * sc, 0));
	head.curveTo(ofPoint(0, -5.5*sc));
	head.curveTo(ofPoint(-5 * sc, -5.5*sc));
	head.curveTo(ofPoint(-0.5 * sc, -1.5*sc));
	head.curveTo(ofPoint(-1, -1.5*sc));
	...
	//Passaro
    /*ofPath head;
	head.curveTo(ofPoint(-10, 2 * sc));
	head.curveTo(ofPoint(-20, 4.5*sc));
	head.curveTo(ofPoint(-7, 3 * sc));
	head.curveTo(ofPoint(-4, 9 * sc));
	head.curveTo(ofPoint(0, 10 * sc));
	head.curveTo(ofPoint(0, 6 * sc));
...
void Shark::setSizeAndSpeed(double sz)
{
	...
	topSpeed = size / 1.2;
	maxVelocityChange = size / 2.5;

	// Chill speed
	if (!isHunting)
	{
		topSpeed = size / 9;
		maxVelocityChange = size / 18;
	}
}
...
void Shark::draw()
{
	...
	// Fish scale (muda a cor e as dimensões do peixe)
	float sc = size;
	float tailSize = 2 * sc;
	float fishLength = 4 * sc;
	float fishHead = tailSize;
...
//		c.setHsb((int)hsb, 255, 255); // rainbow
	...
	ofDrawEllipse(3, 8, sc*1.5, sc*0.5);
	ofDrawEllipse(3, -8, sc*1.5, sc*0.5);
	ofFill();
	ofPopMatrix();
}
